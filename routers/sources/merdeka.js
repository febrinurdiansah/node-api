...
async function fetchCategories() {
  ...
  try {
    ...
    const dataSource = data.endpoints.find(endpoint => endpoint.name === 'merdeka');
    if (!dataSource) {
      throw new Error('Merdeka endpoint not found');
    }
...
async function fetchData(category) {
  const apiUrl = `https://api-berita-indonesia.vercel.app/merdeka/${category}/`;
