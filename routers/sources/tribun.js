...
async function fetchCategories() {
  ...
  try {
    ...
    const dataSource = data.endpoints.find(endpoint => endpoint.name === 'tribun');
    if (!dataSource) {
      throw new Error('Tribun endpoint not found');
    }
...
async function fetchData(category) {
  const apiUrl = `https://api-berita-indonesia.vercel.app/tribun/${category}/`;
