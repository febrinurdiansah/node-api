...
async function fetchCategories() {
  ...
  try {
    ...
    const dataSource = data.endpoints.find(endpoint => endpoint.name === 'jpnn');
    if (!dataSource) {
      throw new Error('Jpnn endpoint not found');
    }
...
async function fetchData(category) {
  const apiUrl = `https://api-berita-indonesia.vercel.app/jpnn/${category}/`;
