...
async function fetchCategories() {
  ...
  try {
    ...
    const dataSource = data.endpoints.find(endpoint => endpoint.name === 'kumparan');
    if (!dataSource) {
      throw new Error('Kumparan endpoint not found');
    }
...
async function fetchData(category) {
  const apiUrl = `https://api-berita-indonesia.vercel.app/kumparan/${category}/`;
